...
function main() {
    ...
    test_lib_1.describe("Validators", function () {
        test_lib_1.describe("required", function () {
            test_lib_1.it("should error on an empty string", function () { test_lib_1.expect(forms_1.Validators.required(new forms_1.Control(""))).toEqual({ "required": true }); });
            test_lib_1.it("should error on null", function () { test_lib_1.expect(forms_1.Validators.required(new forms_1.Control(null))).toEqual({ "required": true }); });
            test_lib_1.it("should not error on a non-empty string", function () { test_lib_1.expect(forms_1.Validators.required(new forms_1.Control("not empty"))).toEqual(null); });
        });
        test_lib_1.describe("compose", function () {
            test_lib_1.it("should collect errors from all the validators", function () {
                ...
                test_lib_1.expect(c(new forms_1.Control(""))).toEqual({ "a": true, "b": true });
            });
        ...
        test_lib_1.describe("controlGroupValidator", function () {
            test_lib_1.it("should collect errors from the child controls", function () {
                ...
                var g = new forms_1.ControlGroup({ "one": one, "two": two });
                test_lib_1.expect(forms_1.Validators.group(g)).toEqual({ "a": [one], "b": [two] });
            });
