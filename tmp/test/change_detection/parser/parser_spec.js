...
function main() {
    ...
    function addPipes(ast, pipes) { return createParser().addPipes(ast, pipes); }
    function emptyLocals() { return new locals_1.Locals(null, collection_1.MapWrapper.create()); }
    ...
    test_lib_1.describe("parser", function () {
        test_lib_1.describe("parseAction", function () {
            test_lib_1.describe("basic expressions", function () {
                test_lib_1.it('should parse numerical expressions', function () { expectEval("1").toEqual(1); });
                test_lib_1.it('should parse strings', function () {
                    expectEval("'1'").toEqual('1');
                    expectEval('"1"').toEqual('1');
                });
                test_lib_1.it('should parse null', function () { expectEval("null").toBe(null); });
                ...
                test_lib_1.it('should parse multiplicative expressions', function () { expectEval("3*4/2%5").toEqual(3 * 4 / 2 % 5); });
                test_lib_1.it('should parse additive expressions', function () { expectEval("3+6-2").toEqual(3 + 6 - 2); });
                ...
                test_lib_1.it('should short-circuit AND operator', function () { expectEval('false && a()', td(function () { throw "BOOM"; })).toBe(false); });
                test_lib_1.it('should short-circuit OR operator', function () { expectEval('true || a()', td(function () { throw "BOOM"; })).toBe(true); });
                test_lib_1.it('should evaluate grouped expressions', function () { expectEval("(1+2)*3").toEqual((1 + 2) * 3); });
                test_lib_1.it('should parse an empty string', function () { expectEval('').toBeNull(); });
            });
            ...
            test_lib_1.describe("member access", function () {
                ...
                test_lib_1.it('should throw when accessing a field on null', function () { expectEvalError("a.a.a").toThrowError(); });
                ...
                test_lib_1.it("should fall back to a regular field read when Locals " +
            ...
            test_lib_1.describe('safe navigation operator', function () {
                test_lib_1.it('should parse field access', function () {
                    expectEval('a?.a', td(td(999))).toEqual(999);
                    expectEval('a.a?.a', td(td(td(999)))).toEqual(999);
                });
                test_lib_1.it('should return null when accessing a field on null', function () { test_lib_1.expect(function () { expectEval('null?.a', td()).toEqual(null); }).not.toThrow(); });
                test_lib_1.it('should have the same priority as .', function () {
                    test_lib_1.expect(function () { expectEval('null?.a.a', td()).toEqual(null); }).toThrowError();
                });
                if (!test_lib_1.IS_DARTIUM) {
                    test_lib_1.it('should return null when accessing a field on undefined', function () {
                        test_lib_1.expect(function () { expectEval('_undefined?.a', td()).toEqual(null); }).not.toThrow();
                    });
                }
                test_lib_1.it('should evaluate method calls', function () { expectEval('a?.add(1,2)', td(td())).toEqual(3); });
                test_lib_1.it('should return null when accessing a method on null', function () {
                    test_lib_1.expect(function () { expectEval('null?.add(1, 2)', td()).toEqual(null); }).not.toThrow();
                });
            });
            test_lib_1.describe("method calls", function () {
                ...
                test_lib_1.it('should throw when no method', function () { expectEvalError("blah()").toThrowError(); });
                ...
                test_lib_1.it('should fall back to the parent context when Locals does not ' +
                    'have the requested method', function () {
                    var locals = new locals_1.Locals(null, collection_1.MapWrapper.create());
                    expectEval("fn()", td(0, 0, 'parent'), locals).toEqual('parent');
                });
            });
            test_lib_1.describe("functional calls", function () {
                test_lib_1.it("should evaluate function calls", function () { expectEval("fn()(1,2)", td(0, 0, function (a, b) { return a + b; })).toEqual(3); });
                test_lib_1.it('should throw on non-function function calls', function () { expectEvalError("4()").toThrowError(new RegExp('4 is not a function')); });
                test_lib_1.it('should parse functions for object indices', function () { expectEval('a[b()]()', td([function () { return 6; }], function () { return 0; })).toEqual(6); });
            });
            ...
            test_lib_1.it("should error when using pipes", function () { expectEvalError('x|blah').toThrowError(new RegExp('Cannot have a pipe')); });
            test_lib_1.it('should pass exceptions', function () {
                test_lib_1.expect(function () {
                    parseAction('a()').eval(td(function () { throw new lang_1.BaseException("boo to you"); }), emptyLocals());
        ...
        test_lib_1.describe("parseBinding", function () {
            test_lib_1.describe("pipes", function () {
                ...
                test_lib_1.it("should parse pipes in the middle of a binding", function () {
                    var exp = parseBinding("user|a|b.name").ast;
                    test_lib_1.expect(exp.name).toEqual("name");
                    test_lib_1.expect(exp.receiver).toBeAnInstanceOf(ast_1.Pipe);
                    test_lib_1.expect(exp.receiver.name).toEqual("b");
                    test_lib_1.expect(exp.receiver.exp).toBeAnInstanceOf(ast_1.Pipe);
                    test_lib_1.expect(exp.receiver.exp.name).toEqual("a");
                });
                test_lib_1.it("should parse pipes with args", function () {
                    var exp = parseBinding("(1|a:2)|b:3").ast;
                    test_lib_1.expect(exp).toBeAnInstanceOf(ast_1.Pipe);
                    test_lib_1.expect(exp.name).toEqual("b");
                    test_lib_1.expect(exp.args[0]).toBeAnInstanceOf(ast_1.LiteralPrimitive);
                    test_lib_1.expect(exp.exp).toBeAnInstanceOf(ast_1.Pipe);
                    test_lib_1.expect(exp.exp.name).toEqual("a");
                    test_lib_1.expect(exp.exp.args[0]).toBeAnInstanceOf(ast_1.LiteralPrimitive);
                    test_lib_1.expect(exp.exp.exp).toBeAnInstanceOf(ast_1.LiteralPrimitive);
                });
                test_lib_1.it('should only allow identifier or keyword as formatter names', function () {
                    ...
                    test_lib_1.expect(function () { return parseBinding('"Foo"|1234'); })
                        .toThrowError(new RegExp('identifier or keyword'));
                    test_lib_1.expect(function () { return parseBinding('"Foo"|"uppercase"'); })
                        .toThrowError(new RegExp('identifier or keyword'));
                });
            });
        ...
        test_lib_1.describe('wrapLiteralPrimitive', function () {
            test_lib_1.it('should wrap a literal primitive', function () {
                test_lib_1.expect(createParser().wrapLiteralPrimitive("foo", null).eval(null, emptyLocals()))
                    .toEqual("foo");
            });
        });
    });
