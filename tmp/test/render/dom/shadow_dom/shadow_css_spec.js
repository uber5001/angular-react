...
function main() {
    test_lib_1.describe('ShadowCss', function () {
        ...
        test_lib_1.it('should handle keyframes rules', function () {
            ...
            var passRe = lang_1.RegExpWrapper.create('@keyframes foo {\\s*0% {\\s*transform:translate\\(-50%\\) scaleX\\(0\\);\\s*}\\s*}');
            test_lib_1.expect(lang_1.RegExpWrapper.test(passRe, s(css, 'a'))).toEqual(true);
        });
        if (dom_adapter_1.DOM.getUserAgent().indexOf('AppleWebKit') > -1) {
            test_lib_1.it('should handle -webkit-keyframes rules', function () {
                ...
                var passRe = lang_1.RegExpWrapper.create('@-webkit-keyframes foo {\\s*0% {\\s*transform:translate\\(-50%\\) scaleX\\(0\\);\\s*}}');
                test_lib_1.expect(lang_1.RegExpWrapper.test(passRe, s(css, 'a'))).toEqual(true);
            });
        }
