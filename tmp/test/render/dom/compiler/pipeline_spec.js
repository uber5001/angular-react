            ...
            var pipeline = new compile_pipeline_1.CompilePipeline([new MockStep(function (parent, current, control) {
                    if (lang_1.isPresent(dom_adapter_1.DOM.getAttribute(current.element, 'viewroot'))) {
                        current.inheritedProtoView =
                            new proto_view_builder_1.ProtoViewBuilder(current.element, api_1.ProtoViewDto.EMBEDDED_VIEW_TYPE);
                    }
                })]);
            var results = pipeline.process(element);
...
var IgnoreCurrentElementStep = (function () {
    ...
    IgnoreCurrentElementStep.prototype.process = function (parent, current, control) {
        var attributeMap = dom_adapter_1.DOM.attributeMap(current.element);
        if (collection_1.MapWrapper.contains(attributeMap, 'ignore-current')) {
